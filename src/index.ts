...
export function getColor(str: string, options: Options = defaults): Color {
  const opts = Object.assign(JSON.parse(JSON.stringify(defaults)), options || {})
  const allowedColorsSet= new Set(opts.shades)
  const excludeColorsSet= new Set(opts.excludeCategories)
  const choiceColors = colors.filter(c => {
    if (excludeColorsSet.has(c.category) || !allowedColorsSet.has(c.shade))
      return false
    return true
  })
  const stringHashNumber = Math.abs(stringToHashCode(str))
  const colorIndex = stringHashNumber % choiceColors.length
  return choiceColors[colorIndex];
}

getColor.rgb = function (str: string, options: Options = defaults): string {
  const color = getColor(str, options);
  return color.rgb;
};

getColor.hex = function (str: string, options: Options = defaults): string {
  const color = getColor(str, options);
  return color.hex;
};

getColor.fontColor = function (str: string, options: Options = defaults): string {
  const color = getColor(str, options)
